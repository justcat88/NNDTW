...
Train_sensor = [Train_sensor Train_sensor Train_sensor]; %Final Training datset
...
for i = 1 : length(test_index) % Loop over every instance in the test set
   ...
   for t = 1:length(Testing_time{i})
       for j = 1:tot_sens_num
           ...
           [predicted_distance{i}(:,t), predicted_class(t) , idx{i}(t, j)] = Classification_Algorithm_DTW(Train_sensor{j},Train_label, classify_this_object); % Using DTW distance
...
% Load the file "predicted_class_sensor_DTW_J" to save time, instead of
% running the DTW code above
% predicted_class_sensor = load('predicted_class_sensor_DTW_J'); 
% predicted_class_sensor = predicted_class_sensor.predicted_class_sensor;
...
for i = 1:length(test_index)
    ...
    FAR(i) = (sum(abs(Testing_label{i}(1:ft(i)-1) - predicted_class_voting{i}(1:ft(i)-1)))/length(Testing_time{i}(1:ft(i)-1))); %Fire alarm rate 
...
result = [EFST' FSTA' FAR'*100]; % Obtain result for each scenario
